...
def handle_callbacks(call):
    ...
    if call.data == "open_case":
        with sqlite3.connect(DB_PATH) as conn:
            ...
            cursor.execute("SELECT cases FROM Users WHERE real_id = ?", (user_id,))
            res = cursor.fetchone()
            current_cases = res[0] if res else 0
            conn.commit()
        ...
        if len(parts) == 4:
            ...
            with sqlite3.connect(DB_PATH) as conn:
                ...
                target_name = res_name[0] if res_name else "Пользователь"
            ...
            target_real_id = int(parts[1])
            cases_to_transfer = int(parts[2])

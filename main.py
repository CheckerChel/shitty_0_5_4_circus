...
token = "8638079016:AAGUx9b4Ih2D6Aew3SsKC-OVPQtqSNo1n_c"
...
def play_coin(message):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        current_time = int(time.time())
        if choice == outcome:
            cursor.execute("UPDATE Users SET ob = ob + ?, wins = wins + ?, time = ? WHERE real_id = ?", (bet, 1, current_time, user_id))
            conn.commit()
            bot.reply_to(message, f"🌹Выпало: {outcome}. Твой выигрыш: {round(bet * 2, 2)} об!")
        else:
            cursor.execute("UPDATE Users SET ob = ob - ?, time = ? WHERE real_id = ?", (bet, current_time, user_id))
            conn.commit()
